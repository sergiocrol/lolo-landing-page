...
const Modal = ({ isShowing, hide, component: Component }) => isShowing ? ReactDOM.createPortal(
  <Fragment>
    <div className="fixed top-0 left-0 z-20 w-screen h-screen bg-black opacity-70" />
    <div className="fixed flex justify-center items-center top-0 left-0 z-30 w-full h-full overflow-x-hidden overflow-y-auto outline-none" aria-modal aria-hidden tabIndex={-1} role="dialog">
      <div className="z-10 bg-yellow relative my-7 mx-auto rounded-sm p-8 h-80 max-h-36 w-11/12 max-w-26">
